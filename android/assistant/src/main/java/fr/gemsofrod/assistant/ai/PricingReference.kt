 ...
 * cours des métaux précieux) — port Kotlin fidèle de backend/app/pricing.py.
 * Source unique importée par SaphirAgent (conversation) et Classifier (tri
 * des emails entrants) : modifier uniquement ce fichier si les tarifs
 * changent.
 */
object PricingReference {
    /** Marge appliquée sur un coût fournisseur (ex. devis de taille lapidaire
     * reçu en PDF) pour calculer le prix proposé au client. */
    const val SUPPLIER_MARGIN_PERCENT = 30

    /** Durée de validité affichée sur tout devis client (le cours des métaux
     * précieux varie). */
    const val QUOTE_VALIDITY_DAYS = 15
